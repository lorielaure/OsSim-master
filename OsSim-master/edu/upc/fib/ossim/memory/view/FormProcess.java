...
public class FormProcess extends FormTemplate { 
	...
	public void init(Vector<Object> values) {
		...
		if (values.size() > 1) spmodel = new SpinnerNumberModel(new Integer(values.get(2).toString()).intValue(), 1, MAX_PROGSIZE, 1);
		...
		if (values.size() > 1) spmodelDuration = new SpinnerNumberModel(new Integer(values.get(3).toString()).intValue(), -1, 100, 1);
