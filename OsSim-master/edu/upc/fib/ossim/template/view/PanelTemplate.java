...
//->
	public PanelTemplate(Presenter presenter, String keyLabelAdd, String keyHelp, boolean time, boolean info, boolean history) { 
		super();
		this.presenter = presenter;
		this.keyLabelAdd = keyLabelAdd;
		this.keyHelp = keyHelp;
...
//->
		if (history) {
			viewHistory = new JButton(Functions.getInstance().createImageIcon("view.png"));
			viewHistory.setToolTipText(Translation.getInstance().getLabel("all_72"));
			viewHistory.setActionCommand("panel_vh");
			viewHistory.addActionListener(presenter);
...
//->
		if (history) viewData.setToolTipText(Translation.getInstance().getLabel("all_72"));
