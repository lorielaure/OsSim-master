...
public class ExerciceDAOImpl implements ExerciceDAO{
	...
	public List<Exercice> getListExercicePublies() {
	    ...
	    try {
	        /* Récupération d'une connexion depuis la Factory */
	        connexion = factoryDAO.getConnection();
	        preparedStatement = initialisationRequetePreparee( connexion,getProperties().getProperty(Constants.REQ_EXERCICE_PUBLIE), false);
	        resultSet = preparedStatement.executeQuery();
	        /* Parcours de la ligne de données de l'éventuel ResulSet retourné */
	        if ( resultSet.next() ) {
	...
	public List<Exercice> getListTestPublies() {
	    ...
	    try {
	        /* Récupération d'une connexion depuis la Factory */
	        connexion = factoryDAO.getConnection();
	        preparedStatement = initialisationRequetePreparee( connexion, getProperties().getProperty(Constants.REQ_TEST_PUBLIE), false);
	        resultSet = preparedStatement.executeQuery();
	        /* Parcours de la ligne de données de l'éventuel ResulSet retourné */
	        if ( resultSet.next() ) {
