...
//->
	private JCheckBox variablePartagee;
	
	public JCheckBox getVariablePartagee() {
		return variablePartagee;
	}

	private JCheckBox verrouCheck;
	private JLabel lverrou;
	private JSpinner verrouSpinner;

	public JCheckBox getVerrouCheck() {
		return verrouCheck;
	}

	public JSpinner getVerrouSpinner() {
		return verrouSpinner;
	}
...
//->    
		//ajout de la variable partagée
		JPanel variable = new JPanel(new FlowLayout(FlowLayout.LEFT));
		//variablePartagee = new JCheckBox(Translation.getInstance().getLabel("pr_26"));
		variablePartagee = new JCheckBox("Shared Variable");
		variablePartagee.setActionCommand("VAR");
		variablePartagee.addActionListener(presenter);
		variablePartagee.setSelected(false);
		variablePartagee.setEnabled(false);
		variable.add(variablePartagee);
		pane.add(variable);
		
		//ajout des ressources (verrous)
		
		//verrouCheck = new JCheckBox(Translation.getInstance().getLabel("pr_55"));
		verrouCheck = new JCheckBox("Semaphore");
		verrouCheck.setEnabled(false);
		verrouCheck.setActionCommand("VER");
		verrouCheck.addActionListener(presenter);
		
		
		SpinnerModel spverrou = new SpinnerNumberModel(0, //initial value
				0, //min
				10, //max
				1);                //step
		verrouSpinner = new JSpinner(spverrou);
		verrouSpinner.setName("verrou");
		verrouSpinner.addChangeListener(presenter);
		verrouSpinner.setEnabled(false);
		
		
		JPanel verrouPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
		verrouPanel.add(verrouCheck);
		verrouPanel.add(verrouSpinner);
		//lverrou = new JLabel(Translation.getInstance().getLabel("pr_56"));
		lverrou = new JLabel("Counter(s)");
		verrouPanel.add(lverrou);
		lverrou.setVisible(false);
		verrouSpinner.setVisible(false);
		pane.add(verrouPanel);
	}
...
//->
		variablePartagee.setText("Shared Variable");
		verrouCheck.setText("Use verrou");
		lverrou.setText("Number");
		this.pack();
	}
	
//->
	/**
	 * Sets verrou size control visibility 
	 * 
	 * @param b control visibility 
	 */
	public void visibleVerrou(boolean b) {
		verrouSpinner.setVisible(b);
		lverrou.setVisible(b);
	}
	 ...
	 * @return	verrou size
	 */
	public int getverrouSize() {
		return (Integer) verrouSpinner.getValue();
	}

	/**
	 * Sets verrou number
	 * 
	 * @param size	verrou number
	 */
	public void setverrouSize(int size) {
		verrouSpinner.setValue(size);
		if(size > 0 ) {
			verrouCheck.setSelected(true);
			visibleVerrou(true);
		}else {
			verrouCheck.setSelected(false);
			visibleVerrou(false);
		}
		((ProcessPresenter) presenter).setVerrouNumber(size);
	}

	/**
	 * select or not the shared variable
	 * @param var
	 */
	public void selectSharedVariable(boolean var) {
		variablePartagee.setSelected(var);
		((ProcessPresenter) presenter).setSharedVariable(var);
	}
} 
